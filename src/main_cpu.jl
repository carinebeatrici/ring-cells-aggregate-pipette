...
include("create_circular_rings_cpu.jl")
using .Create_circular_rings

include("read_saved_state_cpu.jl")
using .Read_saved_state

include("simulate_evolution_cpu.jl")
using .Simulate_evolution

include("plot_hull_and_normal.jl")
    ...
    while !eof(main_pars)
        ...
        if isempty(split(line))
            ...
            if continue_simu == 0
                ...
                x_positions, y_positions, areas = create_circular_rings(total_rings, 
                    particles_per_ring, ring_diameter, offset_x, offset_y)
            else
            ...
            # plot_hull_and_normal(hull_vec, vec_x, vec_y, output_images)
            close(lpxt)
